...
const [{name:firstUser, tasks:[{description:firstTask}]}] = userData;
...
incompletedTask.forEach(n => {
    ...
    console.log("-", n.name + ":", numOfincompleted , "incomplete task");
})
...
const updatedUserData = userData.map(n => {
    if(n.id == 2){
        return {
            ...n,
            role: "Senior Manager",
            tasks:[
                ...
                {
                    taskId: 203,
                    description: "Metor junior Staff",
                    completed: false
                }
            ]
        }
